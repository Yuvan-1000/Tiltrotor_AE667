...
WHERE THESE NUMBERS CAME FROM (so you can cite them / replace them):
  - Twin 3-bladed proprotors, disk loading ~65-90 kg/m^2, and a large
    linear washout (here -32 deg tip-relative) are all sized to be in the
    family of real tiltrotors: Bell XV-15 (R=3.81 m, sigma=0.089-0.10,
    twist ~ -40 deg, hover RPM 589, design GW ~5900-6800 kg) and the
  ...
  - Our aircraft is deliberately smaller than either (a "light utility
    ...
    loading, tip Mach, twist) in the same family as real tiltrotors --
    this is what makes the Task 8 nondimensional benchmarking meaningful.
...
TILTROTOR_GEOM = RotorGeometry(
    ...
    twist_tip=np.radians(-32.0),   # [rad] high washout, tiltrotor-class blade
    n_stations=80,
)
...
COLLECTIVE_RANGE_HOVER_DEG = (2.0, 20.0)   # allowable collective, helicopter mode
...
AIRCRAFT = dict(
    ...
    design_cruise_speed_ms=110.0,   # ~214 kt, representative light-tiltrotor cruise
    ...
    sea_level_power_per_engine_kW=650.0,
)
...
def design_summary():
    ...
    rows = [
        ...
        ("Twist (linear)", f"{np.degrees(geom.twist_root):.1f} to {np.degrees(geom.twist_tip):.1f}", "deg",
         "high washout for combined hover/cruise efficiency (tiltrotor-class)"),
